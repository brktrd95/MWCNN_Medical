 function [net,stats] = cnn_train_(net, imdb, varargin)
...
for t=1:opts.batchSize:numel(subset)
  fprintf('%s %s: epoch %02d / %d: %3d/%3d:', opts.modelName, mode, state.epoch, opts.numEpochs, ...
...
% -------------------------------------------------------------------------
numGpus = numel(opts.gpus) ;if numGpus > 1
...
for ii = 1:numel(batch)
    ...
    switch CurTask
		...
		case 'Deblurring'
			%inputs(:,:,:,ii) = imfilter(labels(:,:,:,ii), kernel);
			inputs(:,:,:,ii) = im2single(imread(fullfile(imdb.imdbPath2, imdb.filepaths(batch(ii)).name )));
        case 'Deblocking'
